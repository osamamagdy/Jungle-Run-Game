...
int CardTwelve::Fee=0;
int CardTwelve::CardPrice=0;
bool CardTwelve::isEntered=0;


//Virtual function to use in copy,cut,paste card actions 


Card* CardTwelve :: Get_My_Copy (CellPosition pos )
{
	Card * pCard = new CardTwelve (pos);

	((CardTwelve*)pCard)->pOwner = pOwner;
	
	((CardTwelve*)pCard)->Fee = Fee;

	((CardTwelve*)pCard)->CardPrice = CardPrice;
	
	((CardTwelve*)pCard)->isEntered = isEntered;
	
	return pCard ;

}
void CardTwelve :: EnableEdit ()
{
	isEntered = 0 ;
}
...
void CardTwelve::ReadCardParameters(Grid * pGrid)
{
	if(!isEntered)       //checks if the user entered the parameters or not
	{
		...
		pOut->ClearStatusBar();
		pGrid->PrintErrorMessage("Enter the price of the card number 12");
		...
		pGrid->PrintErrorMessage("Enter the fee of the card number 12");
...
void CardTwelve::Apply(Grid* pGrid,Player *pPlayer)
{
	...
	{
		pGrid->PrintErrorMessage("Do you want to buy Card 12 ? y/n");
		...
		if(!(strcmp(answer.c_str(),"y")))                 //the player accepts to buy the card
		{
			if(pPlayer->GetWallet()>=CardPrice)           //checks if the player can buy the card or not
			{
				...
				pGrid->PrintErrorMessage("You have sucessfully bought card 12, click to continue");
			}
			else
			{
		...
		if(pOwner!=pPlayer)               //the card is bought and the passed player is not the owner
		{
			...
			pGrid->PrintErrorMessage("You have already bought card 12 previously, click to continue");
}
CardTwelve::~CardTwelve()
{

}
void CardTwelve::ResetCardOwner()
{
	pOwner=NULL;
}
void CardTwelve::Save(ofstream& OutFile, OBJTYPE Type)
{
...
void CardTwelve::SetSavedStatus(bool Save)
{
	isSaved = Save;
}
void CardTwelve::SetEnteredStatus(bool Enter)
{
	isEntered = Enter;
}
