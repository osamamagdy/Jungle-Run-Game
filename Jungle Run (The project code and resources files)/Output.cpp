...
{
	int xpos ;
	///A variable that takes the horizontal cordinates of the 
	if (cellPos.IsValidCell())
	{
		xpos = cellPos.HCell()*UI.CellWidth;
	}
	else 
	{
		xpos = -1 ;
...
{
	int ypos ;
	///A variable that takes the vertical cordinates of the 
	if (cellPos.IsValidCell())
	{
		ypos = UI.ToolBarHeight + cellPos.VCell()*UI.CellHeight;
	}
	else 
	{
		ypos = -1 ;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_COPY_CARD] = "images\\COPY2.jpg";
	MenuItemImages[ITM_CUT_CARD] = "images\\Capture.jpg";
	...
	MenuItemImages[ITM_OPEN_GRID] = "images\\LOAD.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\EDIT.jpg";
...
{
	int cellStartX,fromStartY,toStartY;
	...
	if (GetCellStartX(fromCell)==GetCellStartX(toCell) && GetCellStartY(fromCell)>GetCellStartY(toCell) )
	{
		...
		// Get the start X and Y coordinates of the upper left corner of the fromCell
	          cellStartX = GetCellStartX(fromCell);
	          fromStartY = GetCellStartY(fromCell);

	            if (fromStartY==-1)
	             {
		           fromStartY = UI.ToolBarHeight + 0 * UI.CellHeight;
	             }
	

	// Get the start Y coordinates of the upper left corner of the toCell (the X should be the same as fromCell .. Vertical)
	
           toStartY = GetCellStartY(toCell);

                  if (toStartY==-1)
	            {
		          toStartY = UI.ToolBarHeight + 0 * UI.CellHeight;
	              }
	...
	///TODO: Draw the cross horizontal lines of the ladder using the appropriate coordinates
			...
			for ( y2= y2 + UI.LadderYOffset; y2<y1 ; y2 = y2 + ( 2 * UI.LadderYOffset ) + 1 )
				{
...
{   
	...
	int y1 = fromStartY + UI.CellHeight/2 - UI.LadderYOffset/2;
