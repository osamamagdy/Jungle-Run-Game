...
void RemoveCardFromCell(const CellPosition & pos);

	
	Card *  IsCard (CellPosition pos) const;
	...
	void ResetGame(); //to be used in new game action

	void SaveAll(ofstream&, OBJTYPE);

	void SetIce(int playerNum);

	void SetFire(int playerNum);

	void SetPoison(int playerNum);

	void SetLightning(Player *p);
