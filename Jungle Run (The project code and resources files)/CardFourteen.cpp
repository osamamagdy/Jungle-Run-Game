...
int CardFourteen::Fee=0;
int CardFourteen::CardPrice=0;
bool CardFourteen::isEntered=0;

//Virtual function to use in copy,cut,paste card actions 

Card * CardFourteen :: Get_My_Copy (CellPosition pos )
{
	Card * pCard = new CardFourteen (pos);

	((CardFourteen*)pCard)->pOwner = pOwner;
	
	((CardFourteen*)pCard)->Fee = Fee;

	((CardFourteen*)pCard)->CardPrice = CardPrice;
	
	((CardFourteen*)pCard)->isEntered = isEntered;
	
	return pCard ;

}
void CardFourteen :: EnableEdit ()
{
	isEntered = 0; 
}
...
void CardFourteen::ReadCardParameters(Grid * pGrid)
{
	if(!isEntered)            //checks if the user entered the parameters or not
	{
		...
		pOut->ClearStatusBar();
		pGrid->PrintErrorMessage("Enter the price of the card number 14");
		...
		pGrid->PrintErrorMessage("Enter the fee of the card number 14");
...
void CardFourteen::Apply(Grid* pGrid,Player *pPlayer)
{
	...
	{
		pGrid->PrintErrorMessage("Do you want to buy Card 14 ? y/n");
		...
		if(!(strcmp(answer.c_str(),"y")))                  //the player accepts to buy the card
		{
			if(pPlayer->GetWallet()>=CardPrice)             //checks if the player can buy the card or not
			{
				...
				pGrid->PrintErrorMessage("You have sucessfully bought card 14, click to continue");
			}
			else
			{
		...
		if(pOwner!=pPlayer)               //the card is bought and the passed player is not the owner
		{
			...
			pGrid->PrintErrorMessage("You have already bought card 14 previously, click to continue");
}
CardFourteen::~CardFourteen()
{

}
void CardFourteen::ResetCardOwner()
{
	pOwner=NULL;
}
void CardFourteen::Save(ofstream &OutFile, OBJTYPE Type)
{
...
void CardFourteen::SetSavedStatus(bool Save)
{
	isSaved = Save;
}
void CardFourteen::SetEnteredStatus(bool Enter)
{
	isEntered = Enter;
}
